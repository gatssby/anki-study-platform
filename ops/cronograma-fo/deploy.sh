...
ssh "$REMOTE" "set -euo pipefail; cd '$REMOTE_ROOT'; docker compose config >/dev/null; docker compose build cronograma-fo; docker compose up -d cronograma-fo; sleep 3; curl -fsS http://127.0.0.1:18000/ >/dev/null"
