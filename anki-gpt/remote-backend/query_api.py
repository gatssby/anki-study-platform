...
def find_matching_cards_from_snapshot(notes, params, deck_resolver=None):
...
def card_matches_search(
    ...
    if q:
        blob = " ".join([
            ...
            " ".join(str(v or "") for v in note.get("fields", {}).values()),
        ])
        if q not in normalize_search_text(blob):
            return False
    return True
...
def search_real_cards_from_snapshot(notes, params, deck_resolver=None):
    q_raw = (get_first(params, "text") or get_first(params, "q") or "").strip()
    parsed_deck, parsed_q = parse_deck_query(q_raw)
    q = normalize_search_text(parsed_q) if parsed_q else ""
    deck = normalize_deck_argument_for_snapshot(get_first(params, "deck") or parsed_deck)
    ...
    def do_GET(self):
        try:
            ...
            if path == "/cards/search-real":
                return send_json(self, search_real_cards_from_snapshot(notes, params, deck_resolver))
